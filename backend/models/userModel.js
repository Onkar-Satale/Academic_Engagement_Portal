...
export const UserModel = {
  ...
  getAuthoritySeatsData: async () => {
    // Fetch current holders of Admin (3), Estate Manager (6), Principal (7)
    const [holders] = await db.query(`
      SELECT u.user_id, u.name, u.email, u.department, u.role_id, r.role_name
      FROM user u
      JOIN role r ON u.role_id = r.role_id
      WHERE u.role_id IN (3, 6, 7)
      ORDER BY FIELD(u.role_id, 3, 7, 6)
    `);

    // Fetch candidate faculty members (Active Teachers + current authorities, excluding active Club Mentors/Heads)
    const [facultyCandidates] = await db.query(`
      SELECT u.user_id, u.name, u.email, u.department, u.role_id, r.role_name
      FROM user u
      JOIN role r ON u.role_id = r.role_id
      WHERE r.role_name IN ('Teacher', 'Estate Manager', 'Principal', 'Admin')
